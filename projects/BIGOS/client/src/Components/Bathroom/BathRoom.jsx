...
function BathRoom() {
  ...
  useEffect(() => {
    const fetchProductsByRoom = async () => {
      try {
        const response = await axiosInstance.get(`/api/products/room/BathRoom`);
        setRoomProducts(response.data);
        setLoading(false);
      } catch (error) {
        console.error(`Error fetching products for BathRoom:`, error);
        setError(`Error fetching products for BathRoom`);
        setLoading(false);
      }
    };
  ...
  return (
    <main style={{ position:'absolute', top:0, left:0 }} onMouseMove={mouseMove}>     
      ...
      <Gallery products={roomProducts} roomData={roomsData.Bathroom} />
    )}
    </main>
  )
}
